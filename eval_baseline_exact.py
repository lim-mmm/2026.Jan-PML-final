...
class ScoreNet(nn.Module):
    def __init__(self, marginal_prob_std, channels, embed_dim=256):
        super().__init__()
...
def compute_nn_d1_d2(gen_flat, train_flat, batch=256):
    ...
    M = gen_flat.shape[0]
    train_norm2 = (train_flat ** 2).sum(dim=1)  # [N]
...
def summarize_nn(d1_cpu, d2_cpu, D):
    ...
    return {
        ...
        "d1_min": float(d1_cpu.min().item()),
        "d1_p01": float(torch.quantile(d1_cpu, 0.01).item()),
        "d1_median": float(torch.quantile(d1_cpu, 0.50).item()),
...
def main():
    ...
    ap.add_argument("--run_dir", type=str, default="runs/mnist_ddpm_baseline")
    ap.add_argument("--ckpt", type=str, default="model_last.pt")
    ap.add_argument("--subset_idx", type=str, default="subset_idx.npy",
                    help="If exists in run_dir, use it as the train reference set (e.g., n=2). If not, use full 60K.")
    ap.add_argument("--T", type=int, default=1000)
    ap.add_argument("--channels", type=str, default="32,64,128,256",
                    help="Comma-separated channels, must match checkpoint. Example: 32,64,128,256 or 64,128,256,512")
    ap.add_argument("--nsamples", type=int, default=50000)
    ap.add_argument("--eval_batch", type=int, default=128)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--k", type=float, default=0.33)
    ap.add_argument("--save_topk_nn", type=int, default=32)
    ap.add_argument("--out_json", type=str, default="mem_unified_results.json")
    args = ap.parse_args()
    ...
    has_subset = os.path.exists(subset_idx_path)
    if has_subset:
        idx_ref = np.load(subset_idx_path).astype(int).tolist()
        ref_mode = "subset_idx"
    else:
        idx_ref = list(range(len(ds))) 
        ref_mode = "full_train_60k"

    train_ref = torch.stack([ds[i][0] for i in idx_ref], dim=0).to(device)  # [-1,1]
    train_ref_u8 = x_to_uint8(train_ref)

    train_keys = {}
    for ti in range(train_ref_u8.size(0)):
        key = train_ref_u8[ti].view(-1).cpu().numpy().tobytes()
        train_keys.setdefault(key, []).append(ti)

    channels = [int(x) for x in args.channels.split(",")]
    unet = ScoreNet(lambda t: torch.ones(1, device=device), channels=channels)
    ...
    results = {
        ...
        "train_ref_mode": ref_mode,
        "train_ref_n": int(train_ref_u8.size(0)),
        "note": "Exact match is defined as identical uint8 (0..255) pixel grid after rounding.",
    }

    train_flat = train_ref.view(train_ref.size(0), -1)                # float [-1,1]
    train_flat_u8 = train_ref_u8.view(train_ref_u8.size(0), -1).float()  # float [0,255]
    ...
    exact_gen_indices = []
    exact_hits_train = np.zeros(train_ref_u8.size(0), dtype=np.int64)
    ...
    for start in range(0, M, bs):
        ...
        d1_u, d2_u, nn_u = compute_nn_d1_d2(gen_flat_u8, train_flat_u8.to(device), batch=512)
    ...
    if args.save_topk_nn > 0 and len(top_nn) > 0:
        ...
        save_pairs_grid(nn_gen_keep, train_ref_u8.cpu(), nn_pairs, nn_png,
                        nshow=min(args.save_topk_nn, len(top_nn)))
    ...
    out_path = os.path.join(run_dir, args.out_json)
